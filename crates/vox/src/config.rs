...
impl Config {
    ...
    pub fn build(cli: Cli) -> Result<Config> {
        ...
        Ok(Config {
            ...
            capture: cli.capture.or(file.capture).unwrap_or_else(|| "default".into()),
            playback: cli.playback.or(file.playback).unwrap_or_else(|| "default".into()),
            ...
            expected_loss: cli.expected_loss.or(file.expected_loss).unwrap_or(DEFAULT_EXPECTED_LOSS),
            dtx: cli.dtx.or(file.dtx).unwrap_or(DEFAULT_DTX),
            jitter_ms: cli.jitter_ms.or(file.jitter_ms).unwrap_or(DEFAULT_JITTER_MS),
